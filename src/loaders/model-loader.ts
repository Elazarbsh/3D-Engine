...
export class ModelLoader {
    ...
    public static async loadFromObjectFilePathAsync(fileName: string): Promise<Model> {
        const data = await (await fetch(fileName)).text();
        return this.parseData(data);
    }

    public static async loadFromObjectFileAsync(file: File): Promise<Model> {
        let data = await file.text();
        return this.parseData(data);
    }

    private static parseData(data : string) : Model{
